...
def run_with_timeout(program_path, timeout_seconds=20):
    """
    Exécute le programme dans un processus séparé avec une gestion 
    robuste des chemins Windows.
    """
    # Conversion du chemin en format POSIX (slashs /) pour éviter les erreurs d'échappement
    program_path_safe = os.path.normpath(program_path).replace('\\', '/')
    
    with tempfile.NamedTemporaryFile(suffix=".py", delete=False) as temp_file:
        temp_file_path = temp_file.name
        # Conversion du chemin du fichier de résultats également
        results_path = os.path.normpath(f"{temp_file_path}.results").replace('\\', '/')
        
        # Utilisation de f-strings avec des chemins sécurisés
        script = f"""
import sys
import numpy as np
import os
...
# Utilisation de chemins avec slashs pour éviter SyntaxError sur Windows
program_path = "{program_path_safe}"
results_path = "{results_path}"

sys.path.insert(0, os.path.dirname(program_path))

print(f"Subprocess start. Path: {{program_path}}")
...
"""
        ...
        temp_file.close() # Important de fermer le fichier avant de l'exécuter
    ...
    try:
        process = subprocess.Popen(
            ...
            text=True # Pour éviter le decode() manuel
        )

        try:
            stdout, stderr = process.communicate(timeout=timeout_seconds)
            
            if stdout: print(f"Subprocess stdout: {stdout}")
            if stderr: print(f"Subprocess stderr: {stderr}")
