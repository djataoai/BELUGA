...
class UnloadBeluga(Action):
    ...
    def is_applicable(self, s: State) -> bool:
        ...
        return (s.current_beluga == self.beluga) and (self.jig in s.beluga_contents) and (s.trailer_load.get(self.trailer) is None) and (s.trailer_location.get(self.trailer, ("beluga", "bside"))[1] == "bside")
    def apply(self, s: State) -> State:
        if not self.is_applicable(s):
            raise ValueError("Action not applicable")
        ns = s.copy()
...
def unload_jig_from_beluga(state: State, jig: str) -> List[Action]:
 ...
 # Chercher un trailer vide coté Beluga
    ...
    trailer_name = find_trailer_at(state, "bside", require_empty=True)    
...
if __name__ == "__main__":
    ...
    input_dir = Path("/home/aichatou/ProjetBeluga/belugaModel/instances_bb")
    output_dir = Path("/home/aichatou/ProjetBeluga/belugaModel/res_bb")
